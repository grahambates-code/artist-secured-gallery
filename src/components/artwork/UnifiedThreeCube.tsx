...
interface UnifiedThreeCubeProps {
  color: string;
...
const UnifiedThreeCube = ({ 
  ...
  // Set camera position and target on mount and when data changes
  useEffect(() => {
    if (camera) {
  ...
  }, [camera, cameraPosition.x, cameraPosition.y, cameraPosition.z, position.x, position.y, position.z]);
...
export default UnifiedThreeCube;
